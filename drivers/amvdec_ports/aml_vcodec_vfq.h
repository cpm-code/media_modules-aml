...
{
	return READ_ONCE(q->rp) == READ_ONCE(q->wp);
}

static inline bool vfq_full(struct vfq_s *q)
{
	int wp = READ_ONCE(q->wp);
	int next = (wp == (q->size - 1)) ? 0 : (wp + 1);

	return next == READ_ONCE(q->rp);
}

static inline bool vfq_push(struct vfq_s *q, struct vframe_s *vf)
{
	int wp = READ_ONCE(q->wp);
	int next = (wp == (q->size - 1)) ? 0 : (wp + 1);

	if (next == READ_ONCE(q->rp))
		return false;
...
static inline struct vframe_s *vfq_pop(struct vfq_s *q)
{
	...
	WRITE_ONCE(q->rp, (rp == (q->size - 1)) ? 0 : (rp + 1));
...
{
	int rp = READ_ONCE(q->rp);

	return (vfq_empty(q)) ? NULL : q->pool[rp];
}

static inline int vfq_level(struct vfq_s *q)
{
	int level = READ_ONCE(q->wp) - READ_ONCE(q->rp);
